...
// var allData = [
//     { 
//         imgsrc : 'https://source.unsplash.com/400x200/?water', 
//         name:'zain',
//         age:21, 
//         live:'islamabad',
//         work:'android',
...
//     },
//     { 
//         imgsrc : 'https://source.unsplash.com/400x200/?water,nature', 
//         name:'Osama',
//         age:22, 
//         live:'wah cantt',
//         work:'web',
...
//     },
//     { 
//         imgsrc : 'https://source.unsplash.com/400x200/?ice,water', 
//         name:'afaq',
//         age:20, 
//         live:'rawalpindi',
//         work:'backend',
...
// const candidate = cvIterator(allData);

// function next() {
//     try{
//     const dataa = candidate.next().value;
//     let data = document.querySelector("#data");
//     console.log(dataa);
//     let newData = `<img src="${dataa.imgsrc}">
            
//         <div class="row my-2">
//             <label for="name">Name: ${dataa.name}</label>
//         </div>
        
//         <div class="row my-1">
//             <label for="age">${dataa.age} years old</label>
//         </div>

//         <div class="row my-1">
//             <label for="live">Lives in ${dataa.live}</label>
//         </div>

//         <div class="row my-1">
//             <label for="work">Primary work on ${dataa.work}</label>
//         </div>

//         <div class="row my-1">
//             <label for="technology">Uses ${dataa.framework} framework</label>
//         </div>
//     </div>`;
...
//         data.innerHTML = "<h1> You Have Reached The End Of Records";
//     }
// }

// var text = document.querySelector("#calculate");
...
document.querySelector("#custom").addEventListener("click" , ()=>{
    if (document.querySelector("#custom").checked){
        var l = `
        <div class="mb-3 row">
            <label class="col-sm-2 col-form-label">Parameter 1</label>
            <div class="col-sm-3">
                <input type="text" class="form-control" id="para1" placeholder="Enter Parameter 1 Key">
            </div>
            <div class="col-sm-3">
                <input type="text" class="form-control" id="para2" placeholder="Enter Parameter 1 Value">
            </div>
            <div class="col-sm-1 btn1">
                <button class="btn-primary" onclick="throughEvent()">+</button>
            </div>
        </div>`
...
document.querySelector("#json").addEventListener("click" , ()=> {document.querySelector('#parameters').innerHTML = ""})

function getResult() {
 
    var url = document.querySelector("#url").value;
    var get = document.querySelector("#get").checked;
    var post = document.querySelector("#post").checked;
    var json = document.querySelector("#json").checked;
    var custom = document.querySelector("#custom").checked;
    var para1 = document.querySelector("#para1").value;
    var para2 = document.querySelector("#para2").value;
    var request = document.querySelector("#request").value;
    
    if (get && url != null && json){
        ...
        fetch(url).then((response) => 
            ...
            { 
                console.log(data);
                document.querySelector("#response").value = JSON.stringify(data , undefined , 4); 
            }
        )
    }
    else if (get && url!=null && custom && para1 != null ){
        
        fetch(url).then((response) => 
            {return response.json() } 
         ).then((data) => 
            { 
                if (String(para2).length<=0){
                    let bool = true;
                    document.querySelector("#response").value = "Result From "+ para1 +" Are\n";
                    Array.from(data).forEach((e)=>{
                        document.querySelector("#response").value += `${e[para1]}` +"\n";
                        bool = false;
                    })
                    if(bool){
                        document.querySelector("#response").value = "No Record Found";
                    }
                }
                else{
                    Array.from(data).forEach((e)=>{
                       if (e[para1] == para2){
                            document.querySelector("#response").value = "Record Found :\n"+JSON.stringify(e ,undefined , 4);
                        }
                    })
        ...
        fetch(url , params).then(response=>{ 
            ...
            console.log(data)
            document.querySelector("#response").value = data.message +" \n"+JSON.stringify(data.data , undefined, 4);
        })
    }
}

let c=2;
function throughEvent() {

 let newpara= 
    `<div class="mb-3 row">
        <label class="col-sm-2 col-form-label">Parameter ${c}</label>
        <div class="col-sm-3">
            <input type="text" class="form-control" id="para1" placeholder="Enter Parameter ${c} Key">
        </div>
        <div class="col-sm-3">
            <input type="text" class="form-control" id="para2" placeholder="Enter Parameter ${c} Value">
        </div>
        <div class="col-sm-1 btn1">
            <button class="btn-primary" onclick="throughEvent()">+</button>
        </div>
